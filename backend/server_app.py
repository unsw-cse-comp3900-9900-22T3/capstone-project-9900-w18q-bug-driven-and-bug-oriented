...
class Config(object):
    ...
    MEDIA_ROOT = os.path.join(BASE_DIR, "backend/static/picture") # 这里要修改，改成项目的结构路径！！！
...
def login():
    ...
    if (transfer_data["staff"] is not None) and (transfer_data["key"] is not None):
        ...
        diner_post = transfer_data["diner"]
...
@app.route('/customer/<string:order_id>/hot', methods=["GET"])
def hot_dishes(order_id):
    ...
    for line in category_post:
        line.pop("id")
        line.pop("last_modified")
    return_json = {"orderId": str(order_id_post), "item list": hot_dish_dict, "category_list": category_post}
    return Response(json.dumps(return_json), mimetype="application/json")

@app.route('/customer/<string:order_id>/<string:category_id>', methods=["GET"])
...
def upload_picture(original_local_pitcture_address): #图片转存功能
    ...
    img_path = os.path.join(MEDIA, filename) # 拼接转存路径和新的文件名
    print(img_path)
...
def add_menuitem():
    if request.method == "GET":
        ...
        menu_item_post = Menuitem(dish_id=dish_id_max_cur, categoryID=category_id_post, category_name=category_name_post, title=title_post, description=description_post,
                                    ingredient=ingredient_post, cost=cost_post, calorie=calorie_post, last_modified=datetime.now())
        #menu_item_post.save()
        return {"code": 0}
